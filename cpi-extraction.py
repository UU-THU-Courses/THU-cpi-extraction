...
def main(args):
  ...
  words_out = Bidirectional(LSTM(128, return_sequences=True, dropout=0.5, kernel_initializer='glorot_uniform', recurrent_initializer='glorot_uniform'))(words_out)

  # Position1
  position1_input = Input(shape=(X_train.shape[1],))
  position1_out = Embedding(X_train.shape[1], int(EMBEDDING_DIM/2))(position1_input)
  position1_out = Bidirectional(LSTM(64, return_sequences=True, dropout=0.5, recurrent_initializer='glorot_uniform', kernel_initializer='glorot_uniform'))(position1_out)

  # Position2
  position2_input = Input(shape=(X_train.shape[1],))
  position2_out = Embedding(X_train.shape[1], int(EMBEDDING_DIM/2))(position2_input)
  position2_out = Bidirectional(LSTM(64, return_sequences=True, dropout=0.5, recurrent_initializer='glorot_uniform', kernel_initializer='glorot_uniform'))(position2_out)
